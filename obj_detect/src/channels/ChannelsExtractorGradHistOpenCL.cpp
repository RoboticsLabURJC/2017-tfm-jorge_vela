...
{
  // Preallocate following advice at: https://stackoverflow.com/questions/65709823/why-do-opencv-calls-to-umat-operations-sometimes-take-half-of-a-second-to-comple
  cv::UMat o = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat o_minusHalf = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);            ;
  cv::UMat m = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);            ;
  cv::UMat od = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);            ;
  cv::UMat o0 = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);            ;
  cv::UMat O0_float = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);            ;
  ...
  if ( interpolate )
  {
    ...
    cv::UMat diff = cv::UMat(O0.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  ...
  else
  {
    ...
    M1 = cv::UMat::zeros(M0.rows, M0.cols, CV_32F);
  ...
  )
{
  ...
  std::vector<cv::Mat> Hs_cpu(m_nOrients);
  for (int i=0; i < m_nOrients; i++)
  {
    // GPU -> CPU
    Hs[i].copyTo(Hs_cpu[i]);
  }
...
void
ChannelsExtractorGradHistOpenCL::gradHistUMat
  (
  cv::UMat M,
  cv::UMat O,
  ...
  )
{
  ...
  cv::UMat O0 = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat M0 = cv::UMat(M.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat O0_eq_i = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat O1 = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat M1 = cv::UMat(M.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat O1_eq_i = cv::UMat(O.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  ...
  cv::UMat M0_orient_i = cv::UMat(M.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  cv::UMat M1_orient_i = cv::UMat(M.size(), CV_32FC1, cv::USAGE_ALLOCATE_DEVICE_MEMORY);
  ...
  if ( softBin%2!=0 )
  {
    ...
    {
      // first column
      cv::UMat constant(H[o].rows, 1, CV_32F, cv::Scalar(8.f/7.f));
      cv::multiply(H[o](cv::Range::all(), cv::Range(0,1)), constant, H[o](cv::Range::all(), cv::Range(0,1)));  ;
      // first row
      constant = cv::UMat(1, H[o].cols, CV_32F, cv::Scalar(8.f/7.f));
      cv::divide(H[o](cv::Range(0,1), cv::Range::all()), constant, H[o](cv::Range(0,1), cv::Range::all()));  ;
      // last column
      constant = cv::UMat(H[o].rows, 1, CV_32F, cv::Scalar(8.f/7.f));
      cv::divide(H[o](cv::Range::all(), cv::Range(wb-1,wb)), constant, H[o](cv::Range::all(), cv::Range(wb-1,wb)) );  ;
      // last row
      constant = cv::UMat(1, H[o].cols, CV_32F, cv::Scalar(8.f/7.f));
      cv::divide(H[o](cv::Range(hb-1,hb), cv::Range::all()), constant, H[o](cv::Range(hb-1,hb), cv::Range::all()) );  ;
    }
  }
}
