...
std::vector<cv::Mat>
ChannelsExtractorGradMagPDollar::extractFeatures
  (
  cv::Mat img
  )
{
  ...
  channelsGradMag[0] = cv::Mat::zeros(orig_sz.width, orig_sz.height, CV_32FC1); // M
  channelsGradMag[1] = cv::Mat::zeros(orig_sz.width, orig_sz.height, CV_32FC1); // O
  if (nChannels == 1)
  {
    cv::Mat img_float;
    img.convertTo(img_float, CV_32FC1);
    gradMag(img_float.ptr<float>(),
            channelsGradMag[0].ptr<float>(),
            channelsGradMag[1].ptr<float>(),
  ...
  transpose(channelsGradMag[0], channelsGradMag[0]);
  transpose(channelsGradMag[1], channelsGradMag[1]);
